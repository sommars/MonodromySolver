...
HomotopyPathTrack = new Type of MutableHashTable --Edge and direction
HomotopyThread = new Type of MutableHashTable

newThread = method()
newThread (Nothing, Nothing) := (Path, t) -> (
	T := new HomotopyThread from {
		ThreadTask => t,
		PathTrack => Path,
		ExpectedNewSolCount => 0
	};
	T
)
newThread (HomotopyPathTrack, Task, ZZ) := (Path, t, s) -> (
	T := new HomotopyThread from {
		ThreadTask => t,
		PathTrack => Path,
		ExpectedNewSolCount => s
	};
	T
)
...
addCorrespondence (HomotopyEdge,ZZ,ZZ) := (e,a,b) -> (
	if  e.Correspondence12#?a or e.Correspondence21#?b then
		false
	else ( 
...
trackEdge (HomotopyEdge, Boolean, Thing) := (e, from1to2, batchSize) -> (
	...
	newSols := if #untrackedInds > 0 then (
		...
		if USEtrackHomotopy then
			...
			track(polySystem (gammaHead * head.SpecializedSystem), 
	...
	scan(#untrackedInds, 
		i->(
			...
			if status s =!= Regular then (
				...
				if member(s, tail.PartialSols) then
					b:= position(s,tail.PartialSols) 
				else (
